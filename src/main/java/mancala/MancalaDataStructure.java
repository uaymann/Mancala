...
public class MancalaDataStructure implements Serializable {
    ...
    private int pitPos(int pitNum) {
        /*Runtime exceptions don't need to be declared and are
    ...
    /**
     * Get the pit number the iterator is pointing to.
     *
     * @return The pit number.
     */
    protected int getPitNum() {
        int pitNum = -1;
        if(iteratorPos >= 0 && iteratorPos < 6){
            pitNum = iteratorPos + 1;
        } else if (iteratorPos > 6 && iteratorPos < 13) {
            pitNum = iteratorPos;
        }
        return pitNum;
    }
